...
function render() {
  ...
  if (state.view === "settings") {
    workspace.className = "workspace cols-1";
    listEl.hidden = true; sideEl.hidden = true;
    renderSettings(state, mainEl, () => {
      const fresh = resetState();
      Object.keys(state).forEach((k) => delete state[k]);
      Object.assign(state, fresh);
      persist(); render(); toast("Demo data reset");
    });
    return;
  }
  listEl.hidden = false;
  sideEl.hidden = state.view === "files";
  workspace.className = state.view === "files" ? "workspace cols-2" : "workspace cols-3";
  renderPipeline(state, listEl, (ev) => {
...
document.querySelectorAll(".nav-btn").forEach((btn) => {
  btn.onclick = () => { state.view = btn.dataset.view; persist(); render(); };
});
